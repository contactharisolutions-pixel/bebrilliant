...
// Write the SQL to a local temp file first
const sql = `
DO $$
BEGIN
  IF NOT EXISTS (
...
writeFileSync('migrations/rls_verify.sql', sql);
console.log('✅ SQL file written locally');

// SCP the file to VPS
try {
  execSync('scp -o StrictHostKeyChecking=no migrations/rls_verify.sql root@89.116.33.188:/tmp/rls_verify.sql', { stdio: 'inherit' });
  console.log('✅ File copied to VPS');
} catch(e) {
...
try {
  const result = execSync(
    'ssh -o StrictHostKeyChecking=no root@89.116.33.188 "docker cp /tmp/rls_verify.sql supabase-db:/tmp/rls_verify.sql && docker exec supabase-db psql -U postgres -d postgres -f /tmp/rls_verify.sql"',
    { encoding: 'utf8' }
  );
  console.log('\n📋 Migration output:');
  console.log(result);
