...
import {
    ...
    ShieldCheck, Eye, ToggleLeft, ToggleRight, X, Clock, HelpCircle, ArrowRight
} from 'lucide-react'
...
export default function AffiliatePortalPage() {
    ...
    // Fetch initial platform tenants list
    const fetchTenants = useCallback(async () => {
        try {
            ...
            if (data.tenants) {
                ...
                setTenants(insts)
                if (insts.length > 0 && !selectedTenant) {
                    setSelectedTenant(insts[0].id)
                }
            }
        } catch (e) {
    ...
    // Fetch Economics config for selected tenant
    ...
    // Fetch operational lists (Teachers & Payouts)
    ...
    const runSeed = async () => {
        setSeeding(true)
        try {
            const res = await fetch('/api/owner/settings/affiliate/seed', { method: 'POST' })
            const d = await res.json()
            if (d.success) {
                showToast(`Seeded ${d.seeded.teachers} teachers and mock withdrawal payouts.`, 'success')
                if (tab === 'economics') fetchConfig(); else fetchOperations()
            } else {
    ...
    const handleSaveConfig = async () => {
        ...
        try {
            ...
            if (res.ok) showToast('Affiliate Economics committed successfully.', 'success')
    ...
    const handleUpdateKycStatus = async (teacherId: string, kyc_status: string, status: string) => {
        ...
        try {
            ...
            if (res.ok) {
                showToast('Teacher KYC status updated successfully.', 'success')
                setKycDrawer(false)
                fetchOperations()
    ...
    const handleProcessWithdrawal = async (withdrawalId: string, status: 'paid' | 'rejected') => {
        if (status === 'paid' && !bankReference.trim()) {
            return alert('Enter UTR / Bank reference code for payment records.')
        }
        setSaving(true)
        try {
    ...
    const Toggle = ({ label, desc, val, onChange }: any) => (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 24px', background: '#FFF', borderRadius: 16, border: `1px solid ${P.border}`, marginBottom: 12 }}>
            ...
            <button onClick={() => onChange(!val)} style={{ width: 44, height: 24, borderRadius: 12, background: val ? P.success : P.border, border: 'none', position: 'relative', cursor: 'pointer', transition: '0.2s' }}>
    ...
    return (
        <div style={{ background: P.bg, minHeight: '100vh', padding: '36px 40px', fontFamily: 'var(--font-sans)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 36 }}>
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 900, color: P.brand, textTransform: 'uppercase', letterSpacing: '0.15em' }}>Platform Partner Earnings</span>
                    </div>
                    <h1 style={{ fontSize: 32, fontWeight: 950, color: P.dark, margin: 0, letterSpacing: '-0.03em' }}>Affiliate Economics & Payouts</h1>
                    <p style={{ color: P.muted, margin: '8px 0 0', fontWeight: 600, fontSize: 15 }}>Configure affiliate rewards commissions, approve KYC applications, and manage UTR payouts.</p>
                </div>
                <div style={{ display: 'flex', gap: 12 }}>
                    ...
                    <button onClick={runSeed} disabled={seeding} style={{ display: 'flex', alignItems: 'center', gap: 8, background: P.brandBg, border: `1px solid ${P.brand}20`, borderRadius: 12, padding: '12px 20px', fontSize: 13, fontWeight: 800, color: P.brand, cursor: 'pointer' }}>
                        {seeding ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Settings2 size={16} />}
                        Seed Test Data
                    </button>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, background: '#fff', border: `1px solid ${P.border}`, borderRadius: 18, padding: 6, marginBottom: 32, width: 'fit-content', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
                <button onClick={() => setTab('economics')} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 20px', borderRadius: 12, border: 'none', cursor: 'pointer', transition: 'all 0.2s', background: tab === 'economics' ? P.brand : 'transparent', color: tab === 'economics' ? '#fff' : P.muted, fontSize: 13, fontWeight: 800 }}>
                    <Percent size={15} /> Economics Rules
                </button>
                <button onClick={() => setTab('kyc')} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 20px', borderRadius: 12, border: 'none', cursor: 'pointer', transition: 'all 0.2s', background: tab === 'kyc' ? P.brand : 'transparent', color: tab === 'kyc' ? '#fff' : P.muted, fontSize: 13, fontWeight: 800 }}>
                    <Users size={15} /> Registrations & KYC
                </button>
                <button onClick={() => setTab('withdrawals')} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 20px', borderRadius: 12, border: 'none', cursor: 'pointer', transition: 'all 0.2s', background: tab === 'withdrawals' ? P.brand : 'transparent', color: tab === 'withdrawals' ? '#fff' : P.muted, fontSize: 13, fontWeight: 800 }}>
                    <Banknote size={15} /> Withdrawal Payouts
                </button>
            </div>
                    ...
                    <div>
                        <div style={{ background: '#fff', border: `1px solid ${P.border}`, borderRadius: 20, padding: 20 }}>
                            <div style={{ fontSize: 11, fontWeight: 900, color: P.muted, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
                                <Building size={14} color={P.brand} /> Target Institute
                            </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
                        ...
                        <div style={cardStyle}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
                                ...
                                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 900, color: P.dark }}>Teacher Affiliate Mechanics</h3>
                            </div>
                            <Toggle label="Enable Teacher Affiliates" desc="Allow external staff members to sign up, perform KYC verification, and distribute exams." val={config.enable_affiliate_teacher} onChange={(v: boolean) => setConfig({ ...config, enable_affiliate_teacher: v })} />
                            {config.enable_affiliate_teacher && (
                                <div style={{ background: P.bg, border: `1px solid ${P.border}`, borderRadius: 16, padding: '24px 28px', marginTop: 16 }}>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 16 }}>
                                        <div>
                                            <label style={{ display: 'block', fontSize: 11, fontWeight: 900, color: P.muted, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Commission Reward Basis</label>
                                            <select value={config.teacher_reward_type} onChange={e => setConfig({ ...config, teacher_reward_type: e.target.value })} style={{ width: '100%', padding: '12px 16px', border: `1px solid ${P.border}`, borderRadius: 12, fontSize: 13, fontWeight: 700, color: P.dark, background: '#fff', outline: 'none' }}>
                                                ...
                                                <option value="flat">Flat value amount (₹)</option>
                                            </select>
                                        </div>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
                                        <Input label="Minimum Payout Request threshold" val={config.teacher_min_withdrawal} onChange={(v: string) => setConfig({ ...config, teacher_min_withdrawal: Number(v) })} suffix="₹" />
                                    ...
                                    <div style={{ borderTop: `1px solid ${P.border}`, paddingTop: 20 }}>
                                        <Toggle label="Level 2 Referral overrides" desc="Ecosystem commission reward when teachers invite secondary affiliate teachers." val={config.teacher_level2_enabled} onChange={(v: boolean) => setConfig({ ...config, teacher_level2_enabled: v })} />
                                        {config.teacher_level2_enabled && (
                                            <div style={{ marginTop: 12 }}>
                                                <Input label="Level 2 Override Percentage" val={config.teacher_level2_reward_value} onChange={(v: string) => setConfig({ ...config, teacher_level2_reward_value: Number(v) })} suffix="%" />
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div style={cardStyle}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
                                ...
                                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 900, color: P.dark }}>Student Referral Mechanics</h3>
                            </div>
                            <Toggle label="Enable Student Referral Links" desc="Allow registered students to invite other users and earn credit incentives." val={config.enable_affiliate_student} onChange={(v: boolean) => setConfig({ ...config, enable_affiliate_student: v })} />
                            {config.enable_affiliate_student && (
                                <div style={{ background: P.bg, border: `1px solid ${P.border}`, borderRadius: 16, padding: '24px 28px', marginTop: 16 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#FFFBEB', padding: '10px 14px', borderRadius: 10, border: '1px solid #FDE68A', marginBottom: 20, fontSize: 12, color: '#92400E', fontWeight: 700 }}>
                                        <ShieldCheck size={14} /> Student referral points are stored as non-withdrawable internal wallet credits.
                                    </div>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 16 }}>
                                        <Input label="Credits per Successful conversion" val={config.student_reward_credits} onChange={(v: string) => setConfig({ ...config, student_reward_credits: Number(v) })} suffix="pts" />
                                        <Input label="Max credit incentive limit" val={config.student_max_reward_limit} onChange={(v: string) => setConfig({ ...config, student_max_reward_limit: Number(v) })} suffix="pts" />
                                    </div>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                                        ...
                                        <div>
                                            ...
                                            <select value={config.student_usage_restriction} onChange={e => setConfig({ ...config, student_usage_restriction: e.target.value })} style={{ width: '100%', padding: '12px 16px', border: `1px solid ${P.border}`, borderRadius: 12, fontSize: 13, fontWeight: 700, color: P.dark, background: '#fff', outline: 'none' }}>
                                                <option value="none">Usable anywhere (All exams and syllabus)</option>
                                                <option value="exams_only">Usable strictly for exams fee payment</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                            <button onClick={handleSaveConfig} disabled={saving} style={{ display: 'flex', alignItems: 'center', gap: 8, background: P.brand, color: '#fff', border: 'none', borderRadius: 12, padding: '14px 28px', fontSize: 14, fontWeight: 800, cursor: saving ? 'wait' : 'pointer', boxShadow: `0 8px 20px ${P.brand}25` }}>
                                {saving ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Save size={16} />}
                                {saving ? 'Saving...' : 'Save Architecture Settings'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {tab === 'kyc' && (
                <div style={{ background: '#fff', border: `1px solid ${P.border}`, borderRadius: 24, overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.01)' }}>
                    <div style={{ padding: '24px 28px', borderBottom: `1px solid ${P.border}` }}>
                        <h3 style={{ margin: 0, fontSize: 16, fontWeight: 900, color: P.dark }}>Affiliate Registrations Ledger</h3>
                    </div>
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                            <thead>
                                <tr style={{ borderBottom: `2px solid ${P.border}`, background: P.bg }}>
                                    {['Affiliate Profile', 'Institute/Tenant', 'Wallet Balances', 'KYC Status', 'Status', 'Action'].map(h => (
                            ...
                            <tbody>
                                {teachersList.length === 0 && (
                                    <tr>
                                        <td colSpan={6} style={{ padding: 60, textAlign: 'center', color: P.muted, fontWeight: 700 }}>No registrations found. Seed dummy data above to test.</td>
                                    </tr>
                                )}
                                {teachersList.map(t => (
                                    <tr key={t.id} style={{ borderBottom: `1px solid ${P.border}` }}>
                                        ...
                                        <td style={{ padding: '18px 24px' }}>
                                            <div style={{ fontSize: 13, fontWeight: 800, color: P.dark }}>₹{t.wallet_balance} balance</div>
                                            <div style={{ fontSize: 11, color: P.muted }}>₹{t.wallet_withdrawable} withdrawable</div>
                                        </td>
                                        <td style={{ padding: '18px 24px' }}>
                                            <span style={{
                                        ...
                                        <td style={{ padding: '18px 24px' }}>
                                            <button onClick={() => { setSelectedTeacher(t); setKycDrawer(true) }} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', background: P.brandBg, border: `1px solid ${P.brand}20`, borderRadius: 8, color: P.brand, fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>
            ...
            {/* ── WITHDRAWAL PAYOUTS TAB ── */}
            {tab === 'withdrawals' && (
                <div style={{ background: '#fff', border: `1px solid ${P.border}`, borderRadius: 24, overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.01)' }}>
                    <div style={{ padding: '24px 28px', borderBottom: `1px solid ${P.border}` }}>
                        <h3 style={{ margin: 0, fontSize: 16, fontWeight: 900, color: P.dark }}>Withdrawal Payouts Requests</h3>
                    </div>
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                            <thead>
                                <tr style={{ borderBottom: `2px solid ${P.border}`, background: P.bg }}>
                                    {['Teacher Name', 'Requested Amount', 'TDS Deduction', 'Net Payable', 'Status', 'UTR reference', 'Requested'].map(h => (
                                        <th key={h} style={{ padding: '16px 24px', textAlign: 'left', fontSize: 11, fontWeight: 900, color: P.muted, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {withdrawalsList.length === 0 && (
                                    <tr>
                                        <td colSpan={7} style={{ padding: 60, textAlign: 'center', color: P.muted, fontWeight: 700 }}>No requests logged. Seed dummy data above to test.</td>
                                    </tr>
                                )}
                                {withdrawalsList.map(w => (
                                    <tr key={w.id} style={{ borderBottom: `1px solid ${P.border}` }}>
                                        <td style={{ padding: '18px 24px' }}>
                                            <div style={{ fontWeight: 800, fontSize: 14, color: P.dark }}>{w.affiliate_teachers?.name || 'N/A'}</div>
                                            <div style={{ fontSize: 11, color: P.muted }}>{w.affiliate_teachers?.email}</div>
                                        </td>
                                        <td style={{ padding: '18px 24px', fontSize: 13, fontWeight: 800, color: P.dark }}>₹{w.amount_requested}</td>
                                        <td style={{ padding: '18px 24px', fontSize: 13, color: P.error, fontWeight: 700 }}>-₹{w.tds_deducted}</td>
                                        <td style={{ padding: '18px 24px', fontSize: 13, color: P.success, fontWeight: 850 }}>₹{w.amount_payable}</td>
                                        <td style={{ padding: '18px 24px' }}>
                                            <span style={{
                                        ...
                                        <td style={{ padding: '18px 24px' }}>
                                            {w.status === 'paid' ? (
                                                ...
                                                <button onClick={() => { setSelectedWithdrawal(w); setPayoutDrawer(true) }} style={{ padding: '6px 12px', background: P.successBg, border: 'none', borderRadius: 8, color: P.success, fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>
                                                    Process Payout
                                                </button>
                                            ) : (
            ...
            {/* ── KYC DETAILS DRAWER ── */}
            <SideDrawer isOpen={kycDrawer} onClose={() => setKycDrawer(false)} title="KYC Verification Profile">
                {selectedTeacher && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                        ...
                        <div style={{ borderTop: `1px solid ${P.border}`, paddingTop: 20, display: 'flex', gap: 12 }}>
                            <button onClick={() => handleUpdateKycStatus(selectedTeacher.id, 'rejected', 'suspended')} disabled={saving} style={{ flex: 1, padding: 12, background: '#FEF2F2', color: '#DC2626', border: '1px solid #FECACA', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>
                                Reject KYC Profile
                            </button>
                            <button onClick={() => handleUpdateKycStatus(selectedTeacher.id, 'approved', 'active')} disabled={saving} style={{ flex: 2, padding: 12, background: P.success, color: '#fff', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>
            ...
            {/* ── PAYOUTS PROCESSING DRAWER ── */}
            <SideDrawer isOpen={payoutDrawer} onClose={() => setPayoutDrawer(false)} title="Process Withdrawal Request">
                {selectedWithdrawal && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                        <div>
                            <div style={{ fontSize: 18, fontWeight: 900, color: P.dark }}>{selectedWithdrawal.affiliate_teachers?.name}</div>
                        ...
                        <div style={{ background: P.bg, border: `1px solid ${P.border}`, borderRadius: 16, padding: 20 }}>
                            <div style={{ fontSize: 11, fontWeight: 900, color: P.muted, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 12 }}>Payout Calculations</div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 13, fontWeight: 650, color: P.dark }}>
                                ...
                                <span>₹{selectedWithdrawal.amount_requested}</span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10, fontSize: 13, fontWeight: 650, color: P.error }}>
                                <span>TDS Deducted (Tax):</span>
                                <span>-₹{selectedWithdrawal.tds_deducted}</span>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: `1px solid ${P.border}`, paddingTop: 10, fontSize: 15, fontWeight: 900, color: P.success }}>
                                ...
                                <span>₹{selectedWithdrawal.amount_payable}</span>
                            </div>
                        </div>
                        ...
                        <div style={{ padding: '16px 20px', border: `1px solid ${P.border}`, borderRadius: 14, background: '#FFF7ED' }}>
                            ...
                            <div style={{ fontSize: 13, color: '#9A3412', fontWeight: 600 }}>
                                <div>Bank: <strong>{selectedWithdrawal.affiliate_teachers?.bank_details?.bank_name}</strong></div>
                                <div style={{ marginTop: 4 }}>A/C Number: <strong>{selectedWithdrawal.affiliate_teachers?.bank_details?.account_number}</strong></div>
                                <div style={{ marginTop: 4 }}>IFSC Code: <strong>{selectedWithdrawal.affiliate_teachers?.bank_details?.ifsc}</strong></div>
                            </div>
                        </div>

                        {/* Reference code input */}
                        <div>
                            <label style={{ display: 'block', fontSize: 11, fontWeight: 900, color: P.muted, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Bank UTR / Transaction Reference</label>
                            <input type="text" value={bankReference} onChange={e => setBankReference(e.target.value)} placeholder="e.g. UTR1203491823" style={{ width: '100%', padding: '12px 16px', border: `1px solid ${P.border}`, borderRadius: 12, fontSize: 13, fontWeight: 700, color: P.dark, outline: 'none', boxSizing: 'border-box' }} />
                        </div>

                        {/* Actions */}
                        <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                            <button onClick={() => handleProcessWithdrawal(selectedWithdrawal.id, 'rejected')} disabled={saving} style={{ flex: 1, padding: 12, background: '#FEF2F2', color: '#DC2626', border: '1px solid #FECACA', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>
                                Reject Payout
                            </button>
                            <button onClick={() => handleProcessWithdrawal(selectedWithdrawal.id, 'paid')} disabled={saving} style={{ flex: 2, padding: 12, background: P.brand, color: '#fff', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                                Mark as Paid
                            </button>
                        </div>
                    </div>
