                ...
                for (const section of template.sections) {
                    for (const rule of section.rules) {
                        ...
                        const { data: existingQs } = await supabaseAdmin
                            .from('questions')
                            .select('id')
                            .eq('type', section.section_type.toLowerCase() === 'objective' ? 'objective' : 'subjective')
                        ...
                        if (selectedIds.length < requiredSize) {
                            ...
                            const prompt = `Generate ${neededCount} ${section.section_type} questions of ${rule.question_type} difficulty for the topic of academic excellence. Return ONLY a JSON array of objects with question_text (string), options (object {A,B,C,D} if objective), correct_answer (string), explanation (string).`
                            ...
                            try {
                                ...
                                for (const aiQ of parsed) {
                                    const { data: newQ } = await supabaseAdmin.from('questions').insert([{
                                        tenant_id,
                                        type: section.section_type.toLowerCase(),
                        ...
                        if (selectedIds.length > 0) {
                            const mapping = selectedIds.map((qid, idx) => ({
                                exam_id: exam.id,
                                question_id: qid,
                                question_order: qOrder++,
                                section: section.section_name
                            }))
                            await supabaseAdmin.from('offline_exam_questions').insert(mapping)
                        }
                    }
                }
            }
